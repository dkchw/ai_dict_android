...
class TranslateViewModel(private val repository: LlmRepository, private val database: AppDatabase) : ViewModel() {
    ...
    fun streamTranslation(text: String, source: String, target: String, profileId: Int) {
        viewModelScope.launch {
            ...
            try {
                repository.streamTranslation(text, source, target).collect {
                ...
                val word = Word(profileId = profileId, term = text, language = "$source -> $target", sessionId = sessionId, mode = "translate")
