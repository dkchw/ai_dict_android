...
* **Google Play Protect Compliance & Foreground Service Modernization:** Resolved Play Protect PHA detection ("Diese App könnte schädlich sein") by eliminating intrusive notification deletion interception (`setDeleteIntent` / `ACTION_NOTIFICATION_DISMISSED`) and watchdog active-notification polling loops, aligning the 24/7 background service with standard Android foreground service specifications while strictly maintaining `REQUEST_INSTALL_PACKAGES` and seamless in-app updates.
